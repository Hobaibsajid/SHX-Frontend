import PlayerStats from "../../components/Common/PlayerStats"; // ✅ Import the Coach Stats Component
import React, { useEffect, useState } from "react";
import CoachStats from "../../components/Common/CoachStats"; // ✅ Import the Coach Stats Component
import { useNavigate } from "react-router-dom";
const PlayerDashboard = () => {
  // ✅ Define dummy table data for the coach's game
  const tableData = [
    { team: "My Team", match: 5, win: 3, loss: 2, pts: 6, nrr: "+1.5" },
    { team: "Opponent 1", match: 5, win: 4, loss: 1, pts: 8, nrr: "+2.0" },
    { team: "Opponent 2", match: 5, win: 2, loss: 3, pts: 4, nrr: "-0.5" },
    { team: "Opponent 3", match: 5, win: 1, loss: 4, pts: 2, nrr: "-1.0" },
  ];
...
console.log('ffii',filteredTeams);
          ...
          allMatches.forEach((match) => {
            ...
            matchesByEventMap[eventId].push({
              ...
              team_1: {
                ...
                players: matchAttr.team_1?.players?.map((p) => ({
                  id: p?.id,
                  name: p?.name,
              ...
              team_2: {
                ...
                players: matchAttr.team_2?.players?.map((p) => ({
                  id: p?.id,
                  name: p?.name,
                  // add more fields if needed
                })) || [],
  ...
  const getTeamStats = (matches) => {
    ...
    matches?.forEach((match) => {
      ...
      teams.forEach((team) => {
        const hasPlayer = team?.players?.some((player) => player?.id === playerId);
        if (!team?.id || !hasPlayer) return;
  ...
  return (
    <div className="p-5 h-full bg-[#F4F7FE]">
      ...
      {/* ✅ Table & Upcoming Notifications Section */}
