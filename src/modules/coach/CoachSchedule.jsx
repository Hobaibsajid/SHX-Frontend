        ...
        allTeams.forEach((team) => {
          ...
          allMatches.forEach((match) => {
            ...
            const matchData = {
              ...
              eventStartDate: match.event?.startDate,
              eventEndDate: match.event?.endDate,
              matchtime: match.matchtime,
              game: match.game,
              team1_Score: match.team1_Score,
              team2_Score: match.team2_Score,
              venue: match.venue?.name || "Not Assigned",
              team_1: match.team_1,
              team_2: match.team_2,
              winner: match.winner || null,
              isMyMatch:
                myTeamIds.includes(match.team_1?.id) ||
                myTeamIds.includes(match.team_2?.id),
            };

            if (!matchesByEventMap[eventId]) matchesByEventMap[eventId] = [];
            // Prevent duplicates
            if (!matchesByEventMap[eventId].some((m) => m.id === match.id)) {
    ...
    <div className="p-5 h-full bg-[#F4F7FE]">
      ...
      <div className="w-full max-w-7xl mx-auto">
        {events.map((event) => {
          ...
          return (
            <div key={event.eventId} className="bg-white shadow-md rounded-xl mb-4 p-4">
              <h2
                              ...
                              <tbody className="text-sm text-gray-700">
                                {section.data.map((team) => (
                                  ...
                                  >
                                    ...
                                    <td className="px-5 py-3 font-semibold">{team.team_1?.teamName}</td>
                                    ...
                                    <td className="px-5 py-3 text-center text-gray-400 font-medium">vs</td>
                                    <td className="px-5 py-3">
                                    ...
                                    <td className="px-5 py-3 font-semibold">{team.team_2?.teamName}</td>
                                    <td className="px-5 py-3">
                                      {team.winner?.teamName ? (
                                        ...
                                        <span className="text-gray-400 italic">Not Announced</span>
                                      )}
                                    </td>
                                    <td className="px-5 py-3">
